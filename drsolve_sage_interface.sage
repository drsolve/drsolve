...
set_drsolve_output_dir(path)
    Set the directory used for automatically generated output files.

get_drsolve_output_dir()
...
output_dir="path/to/out"
    Directory for an automatically generated output file.  Defaults to
    `./out`; ignored when `foutput` is supplied explicitly.
...
_default_drsolve_output_dir = "out"
...
def set_drsolve_output_dir(path):
    """
    Set the directory for automatically generated drsolve output files.

    The directory is created on demand.  A per-call ``output_dir`` argument
    takes precedence, and an explicit ``foutput`` takes precedence over both.

    Example
    -------
        set_drsolve_output_dir("results")
    """
    if path is None or not os.fspath(path):
        raise ValueError("output directory path must not be empty")
    global _default_drsolve_output_dir
    _default_drsolve_output_dir = os.fspath(path)


def get_drsolve_output_dir():
    """Return the directory used for automatically generated output files."""
    return _default_drsolve_output_dir
...
def _resolve_output_dir(output_dir):
    """Return a per-call output directory or the configured default."""
    if output_dir is not None:
        if not os.fspath(output_dir):
            raise ValueError("output directory path must not be empty")
...
def _make_output_path(output_dir, prefix="drsolve_", suffix=".out"):
    """Create a unique, persistent output-file path under *output_dir*."""
    output_dir = _resolve_output_dir(output_dir)
    os.makedirs(output_dir, exist_ok=True)
    fd, path = tempfile.mkstemp(prefix=prefix, suffix=suffix, dir=output_dir)
    os.close(fd)
    return path
...
def _run_file_mode(
    ...
    if foutput is None:
        foutput = _make_output_path(output_dir)
...
def DixonRes(
    ...
    finput     : temporary input file path; autogenerated in the system temp directory when None
    foutput    : explicit output file path; takes precedence over output_dir
    output_dir : directory for autogenerated output files; defaults to ./out
...
def DixonSolve(
    ...
    drsolve_path : path to the drsolve executable.
                 If None (default), uses the global default set by
                 set_drsolve_path() (initially "./drsolve").
    finput     : temporary input file; autogenerated in the system temp directory when None
    foutput    : explicit output file path; takes precedence over output_dir
    output_dir : directory for autogenerated output files; defaults to ./out
    debug      : print wrapper-level diagnostics; does not change drsolve verbosity
    live_output : stream drsolve stdout/stderr directly during execution
